...
// Copyright (c) 2013 MehCoin developers and contributors
...
//    Litecoin and modified for MehCoin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated MehCoin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for MehCoin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the MehCoin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "MehCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "MehCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "MehCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "MehCoin is downloading blocks...");
